...
const DocumentForum = (props) => {
  ...
  const searchFile = () => {
    let temp = allFiles.filter((f) => {
      if (f.name.toLowerCase().search(search.toLowerCase()) != -1) {
        return f;
      }
    });
    if (temp.length == 0) toast.error("No Files Found");
    setAllFiles(temp);
  };
  ...
  const fetchGlobalFiles = async () => {
    console.log(props.contract, "cow");
    const getAllFiles = await contract.methods
      .getFilesGlobally(props.contract)
      .call();
